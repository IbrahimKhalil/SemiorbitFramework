...
class SwitchButton extends Field
{


    public $Control = Control::SWITCHBUTTON;
	
	public $Type = DataType::BOOL;

    private static bool $_StyleLoaded = false;
	...
	public function PreRender()
	{
		...
		if ( is_empty($this->Control) ) $this->Control = Control::SWITCHBUTTON;
			
		if ( is_empty($this->Type) ) $this->Type = DataType::BOOL;

		if ( empty($this->Value) && $this->Control = Control::SWITCHBUTTON ) $this->Value = 0;
		
	}


    public static function StyleLoaded(): bool
    {
        return self::$_StyleLoaded;
    }


    public static function MarkStyleLoaded($loaded = true)
    {
        self::$_StyleLoaded = $loaded;
    }
